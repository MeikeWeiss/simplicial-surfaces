\begin{tikzpicture}[vertexPlain=nolabels, edgeStyle=nolabels, faceStyle=nolabels]
    % We need to define the drawing style
	
	% First a tetrahedron
	\begin{scope}[xshift=0cm]
		\coordinate (A) at (0,0);
		\coordinate (B) at (2,0);
		\coordinate (C) at (0.8,1.5);
		\coordinate (D) at (1.9,0.7);

		\draw[face,edge]
			(A) -- (B) -- (C) -- cycle
			(B) -- (C) -- (D) -- cycle;
		\draw[dashed,edge] (A) -- (D);

		\vertexLabelR{A}{below}{1}
		\vertexLabelR{B}{below}{1}
		\vertexLabelR{C}{below}{1}
		\vertexLabelR{D}{below}{1}
	\end{scope}

	% Second: four triangles in the form of a cone
	\begin{scope}[xshift=3cm]
		\input{Image_OpenPyramid.tex}
	\end{scope}

	% Three triangles that share an edge
	\begin{scope}[xshift=7cm]
		\input{Image_ThreeBranchingTriangles.tex}
	\end{scope}

	% A butterfly of triangles
	\begin{scope}[xshift=10cm]
		\input{Image_ButterflyOfTriangles.tex}
	\end{scope}

	% An open cone of two triangles
	\begin{scope}[xshift=0cm, yshift=-3cm]
		\coordinate (A) at (0,0);
		\coordinate (B) at (1.3,0.4);
		\coordinate (C) at (0.4,1.3);

		\draw[face, edge]
			(A) -- (B) to[bend right=45] (C) -- cycle
			(A) -- (B) to[bend left=45] (C) -- cycle;

		\vertexLabelR{A}{below}{1}
		\vertexLabelR{B}{below}{1}
		\vertexLabelR{C}{below}{1}
	\end{scope}

	% A surface from non-triangular shapes
	\begin{scope}[xshift=3cm, yshift=-3cm]
		\coordinate (A) at (0,0);
		\coordinate (B) at (1,0);
		\coordinate (C) at (0.5,0.6);
		\coordinate (D) at (0,1);
		\coordinate (E) at (0.5,1.3);
		\coordinate (F) at (1,1);
		\coordinate (G) at (1.7,0.8);
		\coordinate (H) at (1.8,0.2);

		\draw[face, edge]
			(A) -- (B) -- (C) -- cycle
			(A) -- (C) -- (D) -- cycle
			(D) -- (C) -- (F) -- (E) -- cycle
			(C) -- (F) -- (G) -- (H) -- (B) -- cycle;

		\vertexLabelR{A}{below}{1}
		\vertexLabelR{B}{below}{1}
		\vertexLabelR{C}{below}{1}
		\vertexLabelR{D}{below}{1}
		\vertexLabelR{E}{below}{1}
		\vertexLabelR{F}{below}{1}
		\vertexLabelR{G}{below}{1}
		\vertexLabelR{H}{below}{1}
	\end{scope}

	% Double tetrahedron
	\begin{scope}[xshift=7.5cm, yshift=-2.3cm, scale=0.5]
		\input{Image_TwoJoinedTetrahedrons.tex}
	\end{scope}

	% A triangle with an isolated edge and vertex
	\begin{scope}[xshift=10cm, yshift=-3cm]
		\coordinate (A) at (0,0);
		\coordinate (B) at (1,0);
		\coordinate (C) at (0.5, {sqrt(3)/2});	
		\coordinate (D) at (1.5,1);
		\coordinate (E) at (1.2,0.5);

		\draw[face, edge] (A) -- (B) -- (C) -- cycle;
		\draw[edge] (C) -- (D);

		\vertexLabelR{A}{below}{1}
		\vertexLabelR{B}{below}{1}
		\vertexLabelR{C}{below}{1}
		\vertexLabelR{D}{below}{1}
		\vertexLabelR{E}{below}{1}
			
	\end{scope}
\end{tikzpicture}
